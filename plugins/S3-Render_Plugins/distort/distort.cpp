...
void Distort::onPluginLoad()
{
    ...
    vs->compileSourceFile(g.getPluginPath()+"/../distort/glowing.vert");

    fs = new QOpenGLShader(QOpenGLShader::Fragment, this);
    fs->compileSourceFile(g.getPluginPath()+"/../distort/glowing.frag");
...
bool Distort::paintGL()
{
    ...
    // Volvemos a bindear el programa por defecto (shaders por defecto)
...
void Distort::keyPressEvent(QKeyEvent *key)
{
    if (key->key() == Qt::Key_R) {
        GLWidget & g = *glwidget();
        g.makeCurrent();
